...
<p align="center">
  Agent ist das <strong>Runtime-Betriebssystem</strong> für Agenten-Workflows — eine Infrastruktur, die Modelle, Tools, Speicher und Ausführung abstrahiert, um Agenten einmal zu bauen und überall auszuführen.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritisch_  | Wir sind **nicht verbunden** mit `openagen/agent` oder `agent.org`. Die Domain `agent.org` zeigt derzeit auf den Fork `openagen/agent`, und diese Domain/Repository fälscht unsere offizielle Website/Projekt.                                                                                                                                                                                 | Vertraue keinen Informationen, Binärdateien, Fundraising oder Ankündigungen aus diesen Quellen. Verwende nur [dieses Repository](https://github.com/agent-labs/agent) und unsere verifizierten Social-Media-Konten.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Wichtig_ | Unsere offizielle Website ist jetzt online: [agentlabs.ai](https://agentlabs.ai). Danke für deine Geduld während der Wartezeit. Wir erkennen weiterhin Fälschungsversuche: nimm an keiner Investitions-/Finanzierungsaktivität im Namen von Agent teil, wenn sie nicht über unsere offiziellen Kanäle veröffentlicht wird.                                                                                                                   | Verwende [dieses Repository](https://github.com/agent-labs/agent) als einzige Quelle der Wahrheit. Folge [X (@agentlabs)](https://x.com/agentlabs?s=21), [Telegram (@agentlabs)](https://t.me/agentlabs), [Facebook (Gruppe)](https://www.facebook.com/groups/agentlabs), [Reddit (r/agentlabs)](https://www.reddit.com/r/agentlabs/), und [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) für offizielle Updates. |
...
## Benchmark-Snapshot (Agent vs OpenClaw, Reproduzierbar)
...
> Hinweise: Agent-Ergebnisse werden auf Produktions-Builds mit `/usr/bin/time -l` gemessen. OpenClaw benötigt die Node.js-Runtime (typischerweise ~390 MB zusätzlicher Speicher-Overhead), während NanoBot die Python-Runtime benötigt. PicoClaw und Agent sind statische Binaries. Die oben genannten RAM-Zahlen sind Runtime-Speicher; Build-time-Kompilierungsanforderungen sind höher.

<p align="center">
  <img src="zero-claw.jpeg" alt="Agent vs OpenClaw Vergleich" width="800" />
</p>
...
- `agent --help`: Echtzeit ca. `0.02s`, maximaler Speicherbedarf ~`3.9 MB`
- `agent status`: Echtzeit ca. `0.01s`, maximaler Speicherbedarf ~`4.1 MB`
...
Das `bootstrap.sh`-Skript installiert Rust, klont Agent, kompiliert es und richtet deine anfängliche Entwicklungsumgebung ein:
...
2. Das Agent-Repository klonen
3. Agent im Release-Modus kompilieren
4. `agent` in `~/.cargo/bin/` installieren
5. Die Standard-Workspace-Struktur in `~/.agent/workspace/` erstellen
6. Eine Startkonfigurationsdatei `~/.agent/workspace/config.toml` generieren

Nach dem Bootstrap lade deine Shell neu oder führe `source ~/.cargo/env` aus, um den `agent`-Befehl global zu verwenden.
...
1. Konfiguriere deine KI-Provider in `~/.agent/workspace/config.toml`
2. Sieh dir die [Konfigurationsreferenz](docs/config-reference.md) für erweiterte Optionen an
3. Starte den Agent: `agent agent start`
4. Teste über deinen bevorzugten Channel (siehe [Channel-Referenz](docs/channels-reference.md))

## Konfiguration

Bearbeite `~/.agent/workspace/config.toml`, um Provider, Channels und Systemverhalten zu konfigurieren.
...
Agent unterstützt zwei Code-Ausführungs-Backends:
...
agent init                # Initialisiert einen neuen Workspace
agent status              # Zeigt Daemon/Agent-Status
agent config validate     # Überprüft config.toml Syntax und Werte

# Daemon-Verwaltung
agent daemon start        # Startet den Daemon im Hintergrund
agent daemon stop         # Stoppt den laufenden Daemon
agent daemon restart      # Startet den Daemon neu (Config-Neuladen)
agent daemon logs         # Zeigt Daemon-Logs

# Agent-Verwaltung
agent agent start         # Startet den Agent (erfordert laufenden Daemon)
agent agent stop          # Stoppt den Agent
agent agent restart       # Startet den Agent neu (Config-Neuladen)

# Pairing-Operationen
agent pairing init        # Generiert ein neues Pairing-Geheimnis
agent pairing rotate      # Rotiert das bestehende Pairing-Geheimnis

# Tunneling (für öffentliche Exposition)
agent tunnel start        # Startet einen Tunnel zum lokalen Daemon
agent tunnel stop         # Stoppt den aktiven Tunnel

# Diagnose
agent doctor              # Führt System-Gesundheitsprüfungen durch
agent version             # Zeigt Version und Build-Informationen
```
...
Wenn Agent deine Arbeit hilft und du die kontinuierliche Entwicklung unterstützen möchtest, kannst du hier spenden:
...
**Dies ist das einzige offizielle Agent-Repository:**

> <https://github.com/agent-labs/agent>

Jedes andere Repository, Organisation, Domain oder Paket, das behauptet "Agent" zu sein oder eine Verbindung zu Agent Labs zu implizieren, ist **nicht autorisiert und nicht mit diesem Projekt verbunden**. Bekannte nicht autorisierte Forks werden in [TRADEMARK.md](TRADEMARK.md) aufgeführt.

Wenn du auf Fälschung oder Markenmissbrauch stößt, bitte [öffne ein Issue](https://github.com/agent-labs/agent/issues).

---

## Lizenz

Agent ist doppelt lizenziert für maximale Offenheit und Contributorschutz:
...
Der Name **Agent** und das Logo sind eingetragene Marken von Agent Labs. Diese Lizenz gewährt keine Erlaubnis, sie zu verwenden, um Befürwortung oder Verbindung zu implizieren. Siehe [TRADEMARK.md](TRADEMARK.md) für erlaubte und verbotene Verwendungen.
...
- Neuer `Skill` → `~/.agent/workspace/skills/<n>/`

---

**Agent** — Null Overhead. Null Kompromiss. Deploy überall. Tausche alles. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#agent-labs/agent&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Stern-Historie-Diagramm" src="https://api.star-history.com/svg?repos=agent-labs/agent&type=date&legend=top-left" />
    </picture>
  </a>
</p>
