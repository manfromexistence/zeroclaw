...
Rules for pull request quality, attribution, privacy, and handoff in Agent.
...
- If identity-like context is unavoidable, use Agent-scoped roles/labels only (e.g., `AgentAgent`, `AgentOperator`, `agent_user`).
- Recommended identity-safe naming palette:
    - actor labels: `AgentAgent`, `AgentOperator`, `AgentMaintainer`, `agent_user`
    - service/runtime labels: `agent_bot`, `agent_service`, `agent_runtime`, `agent_node`
    - environment labels: `agent_project`, `agent_workspace`, `agent_channel`
