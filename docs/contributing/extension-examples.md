...
Agent's architecture is trait-driven and modular.
...
```rust
// In your crate: use agent::tools::traits::{Tool, ToolResult};
...
Channels let Agent communicate through any messaging platform.
...
```rust
// In your crate: use agent::channels::traits::{Channel, ChannelMessage, SendMessage};
...
Providers are LLM backend adapters. Each provider connects Agent to a different model API.
...
```rust
// In your crate: use agent::providers::traits::Provider;
...
```rust
// In your crate: use agent::memory::traits::{Memory, MemoryEntry, MemoryCategory};
