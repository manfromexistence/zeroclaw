...
```bash
# Check config
cat ~/.agent/config.toml | grep -A 5 telegram
...
| Health check | <5s | `time agent channel doctor` |
...
5. **Monitor logs** for any issues:
   ...
   agent daemon
   # or
   RUST_LOG=info agent channel start
   ```
...
- Issues: https://github.com/agent-labs/agent/issues
