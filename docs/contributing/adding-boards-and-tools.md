# Adding Boards and Tools — Agent Hardware Guide

This guide explains how to add new hardware boards and custom tools to Agent.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.agent/config.toml)
agent peripheral add nucleo-f401re /dev/ttyACM0
agent peripheral add arduino-uno /dev/cu.usbmodem12345
agent peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, Agent can index PDF files:
...
2. **Add to config** — `agent peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `agent peripheral list` | List configured boards |
| `agent peripheral add <board> <path>` | Add board (writes config) |
| `agent peripheral flash` | Flash Arduino firmware |
| `agent peripheral flash-nucleo` | Flash Nucleo firmware |
| `agent hardware discover` | List USB devices |
| `agent hardware info` | Chip info via probe-rs |
