...
Agent supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export AGENT_API_KEY="your-api-key"
agent agent
```

## llama.cpp Server (Recommended Local Setup)

Agent includes a first-class local provider for `llama-server`:
...
```bash
agent models refresh --provider llamacpp
agent agent -m "hello"
```

You do not need to export `AGENT_API_KEY=dummy` for this flow.

## SGLang Server

Agent includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
agent models refresh --provider sglang
agent agent -m "hello"
```
...
Agent includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
agent models refresh --provider vllm
agent agent -m "hello"
```
