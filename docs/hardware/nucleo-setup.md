# Agent on Nucleo-F401RE — Step-by-Step Guide

Run Agent on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

Agent can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
Agent includes everything for Nucleo-F401RE:
...
| Flash command | `agent peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
| 5 | `agent daemon` or `agent agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `agent peripheral list` to verify.
