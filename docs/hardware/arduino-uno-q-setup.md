# Agent on Arduino Uno Q — Step-by-Step Guide

Run Agent on the Arduino Uno Q's Linux side. Telegram works over WiFi; GPIO control uses the Bridge (requires a minimal App Lab app).

---

## What's Included (No Code Changes Needed)

Agent includes everything needed for Arduino Uno Q. **Clone the repo and follow this guide — no patches or custom code required.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `agent peripheral setup-uno-q` deploys the Bridge via scp + arduino-app-cli |
...
## Phase 2: Install Agent on Uno Q
...
# Clone agent (or scp your project)
...
**At this point:** Telegram chat works. Send messages to your bot — Agent responds. No GPIO yet.

---

## Phase 5: GPIO via Bridge (Agent Handles It)

Agent includes the Bridge app and setup command.

### 5.1 Deploy Bridge App

**From your Mac** (with agent repo):
```bash
...
Now when you message your Telegram bot *"Turn on the LED"* or *"Set pin 13 high"*, Agent uses `gpio_write` via the Bridge.
...
| 8 | Edit `~/.agent/config.toml` (add Telegram bot_token) |
| 9 | `agent daemon --host 127.0.0.1 --port 42617` |
...
- **"command not found: agent"** — Use full path: `/usr/local/bin/agent` or ensure `~/.cargo/bin` is in PATH.
...
- **GPIO commands ignored** — Ensure Bridge app is running (`agent peripheral setup-uno-q` deploys and starts it). Config must have `board = "arduino-uno-q"` and `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Use `default_provider = "glm"` or `"zhipu"` with `GLM_API_KEY` in env or config. Agent uses the correct v4 endpoint.
