...
This document provides visual representations of Agent's architecture, execution modes, and data flows.

---

## 1. Execution Modes

**Ways Agent can be run:**

```mermaid
flowchart TD
    Start[agent CLI] --> Onboard[onboard<br/>Setup wizard]
...
```mermaid
flowchart TB
    Start[[agent daemon]] --> SpawnComponents
...
```mermaid
flowchart TB
    Client[HTTP Client] --> Gateway[Agent Gateway]
...
flowchart TB
    ...
    Defaults --> EnvOverrides[apply_env_overrides<br/>AGENT_* env vars]
...
*Generated for Agent v0.1.0 - Architecture Documentation*
