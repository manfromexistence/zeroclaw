...
本指南解释如何向 Agent 添加新的硬件开发板和自定义工具。

## 快速开始：通过 CLI 添加开发板

```bash
# 添加开发板（更新 ~/.agent/config.toml）
agent peripheral add nucleo-f401re /dev/ttyACM0
agent peripheral add arduino-uno /dev/cu.usbmodem12345
agent peripheral add rpi-gpio native   # 用于树莓派 GPIO（Linux）
...
使用 `rag-pdf` 特性时，Agent 可以索引 PDF 文件：
...
2. **添加到配置** — `agent peripheral add my-board /dev/ttyUSB0`
...
| 命令 | 描述 |
|---------|-------------|
| `agent peripheral list` | 列出已配置的开发板 |
| `agent peripheral add <board> <path>` | 添加开发板（写入配置） |
| `agent peripheral flash` | 烧录 Arduino 固件 |
| `agent peripheral flash-nucleo` | 烧录 Nucleo 固件 |
| `agent hardware discover` | 列出 USB 设备 |
| `agent hardware info` | 通过 probe-rs 获取芯片信息 |
