...
- 如果不可避免需要类似身份的上下文，仅使用 Agent 范围的角色/标签（例如 `AgentAgent`、`AgentOperator`、`agent_user`）。
- 推荐的身份安全命名调色板：
    - 参与者标签：`AgentAgent`、`AgentOperator`、`AgentMaintainer`、`agent_user`
    - 服务/运行时标签：`agent_bot`、`agent_service`、`agent_runtime`、`agent_node`
    - 环境标签：`agent_project`、`agent_workspace`、`agent_channel`
