...
本指南介绍 Agent 的原生 Nextcloud Talk 集成。
...
- 设置 `AGENT_NEXTCLOUD_TALK_WEBHOOK_SECRET` 时会覆盖 `webhook_secret`。
...
- Agent 忽略来自机器人的 webhook 事件（`actorType = bots`）。
- Agent 忽略非消息/系统事件。
