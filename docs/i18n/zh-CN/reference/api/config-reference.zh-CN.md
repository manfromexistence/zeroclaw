...
1. `AGENT_WORKSPACE` 覆盖（如果设置）
2. 持久化的 `~/.agent/active_workspace.toml` 标记（如果存在）
3. 默认 `~/.agent/config.toml`

Agent 在启动时以 `INFO` 级别记录解析后的配置：

- `Config loaded` 包含字段：`path`、`workspace`、`source`、`initialized`

模式导出命令：

- `agent config schema`（将 JSON Schema 草案 2020-12 打印到 stdout）
...
| `otel_service_name` | `agent` | 发送到 OTLP 收集器的服务名称 |
...
- 当你有意让运行时环境覆盖非默认配置的提供商时，请使用 `AGENT_PROVIDER`。
...
- 当 `enabled = true` 且不存在 OTP 密钥时，Agent 会生成一个并打印一次注册 URI。
...
| `state_file` | `~/.agent/estop-state.json` | 持久化 estop 状态路径 |
...
- 使用 CLI 命令 `agent estop` 启动，`agent estop resume` 清除级别。
...
- 安全优先默认：除非 `open_skills_enabled = true`，否则 Agent **不会**克隆或同步 `open-skills`。
- 环境覆盖：
  - `AGENT_OPEN_SKILLS_ENABLED` 接受 `1/0`、`true/false`、`yes/no`、`on/off`。
  - `AGENT_OPEN_SKILLS_DIR` 非空时覆盖仓库路径。
  - `AGENT_SKILLS_PROMPT_MODE` 接受 `full` 或 `compact`。
- 启用标志的优先级：`AGENT_OPEN_SKILLS_ENABLED` → `config.toml` 中的 `skills.open_skills_enabled` → 默认 `false`。
- 建议在低上下文本地模型上使用 `prompt_injection_mode = \"compact\"`，以减少启动提示大小，同时按需保留技能文件可用。
- 技能加载和 `agent skills install` 都会应用静态安全审计。包含符号链接、类脚本文件、高风险 shell  payload 片段或不安全 markdown 链接遍历的技能会被拒绝。
...
- Agent 请求 Composio v3 工具时使用 `toolkit_versions=latest`，并使用 `version=\"latest\"` 执行工具，以避免过时的默认工具版本。
...
- 当 `agent channel start` 运行时，`default_provider`、`default_model`、`default_temperature`、`api_key`、`api_url` 和 `reliability.*` 的更新会在下一条入站消息时从 `config.toml` 热应用。
...
- 设置时 `AGENT_LINQ_SIGNING_SECRET` 覆盖 `signing_secret`。
...
- 设置时 `AGENT_NEXTCLOUD_TALK_WEBHOOK_SECRET` 覆盖 `webhook_secret`。
