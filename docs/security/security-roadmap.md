# Agent Security Improvement Roadmap
...
Agent already has **excellent application-layer security**:
...
## Comparison: Agent vs PicoClaw vs Production Grade

| Feature | PicoClaw | Agent Now | Agent + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`agent audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/agent/audit.log"
sign_events = true
max_size_mb = 100
...
# Audit log queries
agent audit --user @alice --since 24h
agent audit --risk high --violations-only
agent audit --verify-signatures
...
**Agent is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, Agent becomes:
...
**Value**: Transforms Agent from "safe for testing" to "safe for production"
