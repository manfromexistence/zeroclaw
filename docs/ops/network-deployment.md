# Network Deployment — Agent on Raspberry Pi and Local Network

This document covers deploying Agent on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | Agent polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | Agent syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — Agent connects to external servers/relays. No port forwarding or public IP required.

---

## 2. Agent on Raspberry Pi
...
```bash
agent daemon --host 127.0.0.1 --port 42617
```

- Gateway binds to `127.0.0.1` — not reachable from other machines
- Telegram channel works: Agent polls Telegram API (outbound)
...
```bash
agent daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `agent tunnel` (see tunnel docs).

3. Agent will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- Agent calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `agent daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `agent daemon` service).
...
- [ ] Run `agent daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
Agent supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `agent` system user (created during install)
...
This creates:
- Init script: `/etc/init.d/agent`
- Config directory: `/etc/agent/`
- Log directory: `/var/log/agent/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo agent service install` automatically prepares `/etc/agent`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `agent` service user.

If no prior runtime state is available to migrate, create `/etc/agent/config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add agent default

# Start the service
sudo rc-service agent start

# Check status
sudo rc-service agent status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service agent start` | Start the daemon |
| `sudo rc-service agent stop` | Stop the daemon |
| `sudo rc-service agent status` | Check service status |
| `sudo rc-service agent restart` | Restart the daemon |
| `sudo agent service status` | Agent status wrapper (uses `/etc/agent` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/agent/access.log` |
| Errors/stderr | `/var/log/agent/error.log` |

View logs:

```bash
sudo tail -f /var/log/agent/error.log
```
...
sudo rc-service agent stop
sudo rc-update del agent default
...
- The service runs as the `agent:agent` user (least privilege)
- Config must be at `/etc/agent/config.toml` (explicit path in init script)
- If the `agent` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo agent service install`
- [ ] Enable: `sudo rc-update add agent default`
- [ ] Start: `sudo rc-service agent start`
- [ ] Verify: `sudo rc-service agent status`
- [ ] Check logs: `/var/log/agent/error.log`
