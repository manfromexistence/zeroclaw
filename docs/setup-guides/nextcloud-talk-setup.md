...
This guide covers native Nextcloud Talk integration for Agent.
...
Add this section in `~/.agent/config.toml`:
...
agent daemon
# or
agent gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, Agent verifies:
...
- Agent ignores bot-originated webhook events (`actorType = bots`).
- Agent ignores non-message/system events.
...
3. Confirm Agent receives and replies in the same room.
