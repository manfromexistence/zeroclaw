...
Agent supports native integration with Mattermost via its REST API v4. This integration is ideal for self-hosted, private, or air-gapped environments where sovereign communication is a requirement.
...
| `mention_only` | (Optional) When `true`, only messages that explicitly mention the bot username (for example `@agent-bot`) are processed. Default: `false`. |

## Threaded Conversations

Agent supports Mattermost threads in both modes:
- If a user sends a message in an existing thread, Agent always replies within that same thread.
...
When `mention_only = true`, Agent applies an extra filter after `allowed_users` authorization:
