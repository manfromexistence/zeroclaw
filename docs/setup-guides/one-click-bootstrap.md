...
This page defines the fastest supported path to install and initialize Agent.
...
Default behavior is **app-only** (build/install Agent) and expects existing Rust toolchain.
...
This builds a local Agent image and launches onboarding inside a container while
persisting config/workspace to `./.agent-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
the installer auto-falls back to `podman`. You can also set `AGENT_CONTAINER_CLI`
explicitly (for example: `AGENT_CONTAINER_CLI=podman ./install.sh --docker`).
...
Docker tag (`AGENT_DOCKER_IMAGE`, default: `agent-bootstrap:local`); if missing,
it pulls `ghcr.io/agent-labs/agent:latest` and tags it locally before running.
...
are persisted in the data directory (default: `./.agent-docker`, or `~/.agent-docker`
when bootstrapping via `curl | bash`). You can override this path with `AGENT_DOCKER_DATA_DIR`.
...
The simplest way to run Agent long-term in Docker/Podman is with the provided
`docker-compose.yml` at the repository root. It uses a named volume (`agent-data`)
...
If you installed via `./install.sh --docker` and want to reuse the `.agent-docker`
...
podman run -d --name agent \
  ...
  --userns keep-id \
  --user "$(id -u):$(id -g)" \
  -v "$PWD/.agent-docker/.agent:/agent-data/.agent:Z" \
  -v "$PWD/.agent-docker/workspace:/agent-data/workspace:Z" \
  -e HOME=/agent-data \
  -e AGENT_WORKSPACE=/agent-data/workspace \
  -p 42617:42617 \
  agent-bootstrap:local \
...
# Remove the container (data in volumes/.agent-docker is preserved)
docker rm agent
...
saved in `.agent-docker/.agent/config.toml` and do not need to be passed again.
...
```bash
AGENT_API_KEY="sk-..." AGENT_PROVIDER="openrouter" ./install.sh
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/agent-labs/agent:latest`)
- `--skip-install`
- `--provider <id>`
