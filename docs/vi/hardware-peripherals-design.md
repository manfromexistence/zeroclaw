# Thiết kế Hardware Peripherals — Agent

Agent cho phép các vi điều khiển (MCU) và máy tính nhúng (SBC) **phân tích lệnh ngôn ngữ tự nhiên theo thời gian thực**, tổng hợp code phù hợp với từng phần cứng, và thực thi tương tác với ngoại vi trực tiếp.

## 1. Tầm nhìn

**Mục tiêu:** Agent đóng vai trò là AI agent có hiểu biết về phần cứng, cụ thể:
...
**Hình dung trực quan:** Agent = bộ não hiểu phần cứng. Ngoại vi = tay chân mà nó điều khiển.
...
Agent chạy **trực tiếp trên thiết bị**. Board khởi động server gRPC/nanoRPC và giao tiếp với ngoại vi ngay tại chỗ.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  Agent on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. Agent truy xuất tài liệu theo board (ví dụ: bản đồ GPIO của ESP32)
...
Agent chạy trên **máy chủ** và duy trì kết nối phần cứng tới thiết bị mục tiêu. Dùng cho phát triển, kiểm tra nội tâm, và nạp firmware.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  Agent on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. Agent nhận diện phần cứng đang kết nối (VID/PID, kiến trúc)
...
2. Agent ghi/nạp firmware qua OpenOCD hoặc probe-rs
3. Xác nhận thành công

**Hoặc:**
1. Agent tự phát hiện: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|-----------|-------------|---------------|
| Agent chạy trên | Thiết bị (ESP32, RPi) | Máy chủ (Mac, Linux) |
...
Máy chủ chạy Agent; ngoại vi chạy firmware tối giản. JSON đơn giản qua serial.

### Chế độ B: RPi làm Host (Native GPIO)

Agent trên Pi; GPIO qua rppal hoặc sysfs. Không cần firmware riêng.
...
agent agent --mode edge

# Host-Mediated: connect to USB/J-Link target
agent agent --peripheral nucleo-f401re:/dev/ttyACM0
agent agent --probe jlink
...
board = "esp32"
transport = "wifi"
# Edge-Native: Agent runs on ESP32
```
...
1. **Khởi động:** Agent nạp config, đọc `peripherals.boards`.
...
Dành cho RPC có kiểu dữ liệu, độ trễ thấp giữa Agent và các peripheral:
...
- **agent-firmware** hoặc **agent-peripheral** — một crate/workspace riêng biệt.
...
- Người dùng nạp lên board; Agent kết nối và tự phát hiện khả năng.

## 9. Các giai đoạn triển khai

### Phase 1: Skeleton ✅ (Hoàn thành)

- [x] Thêm trait `Peripheral`, config schema, CLI (`agent peripheral list/add`)
- [x] Thêm flag `--peripheral` cho agent
- [x] Ghi tài liệu vào AGENTS.md

### Phase 2: Host-Mediated — Phát hiện phần cứng ✅ (Hoàn thành)

- [x] `agent hardware discover`: liệt kê thiết bị USB (VID/PID)
- [x] Board registry: ánh xạ VID/PID → kiến trúc, tên (ví dụ: Nucleo-F401RE)
- [x] `agent hardware introspect <path>`: memory map, danh sách peripheral
...
- [x] Agent trên Raspberry Pi (native GPIO qua rppal)
...
- [ ] Agent *chạy trực tiếp trên* ESP32 (WiFi + LLM, edge-native) — tương lai
...
- Chạy Agent đầy đủ *trực tiếp trên* STM32 bare-metal (không có WiFi, RAM hạn chế) — dùng Host-Mediated thay thế
...
> *"Các board như ESP, Raspberry Pi, hoặc các board có WiFi có thể kết nối với LLM (Gemini hoặc mã nguồn mở). Agent chạy trên thiết bị, tạo gRPC riêng, khởi động nó, và giao tiếp với ngoại vi. Người dùng hỏi qua WhatsApp: 'di chuyển cánh tay X' hoặc 'bật LED'. Agent lấy tài liệu chính xác, viết code, thực thi, lưu trữ tối ưu, chạy, và bật LED — tất cả trên board phát triển.*
>
> *Với STM Nucleo kết nối qua USB/J-Link/Aardvark vào Mac: Agent từ Mac truy cập phần cứng, cài đặt hoặc ghi những gì cần thiết lên thiết bị, và trả kết quả. Ví dụ: 'Hey Agent, những địa chỉ khả dụng/đọc được trên thiết bị USB này là gì?' Nó có thể tự tìm ra thiết bị nào đang kết nối ở đâu và đưa ra gợi ý."*
