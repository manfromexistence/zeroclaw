...
- `cargo check` / `cargo build` dừng lâu ở `Checking agent`
...
### Không tìm thấy lệnh `agent` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `agent`
...
- Dừng các tiến trình `agent daemon` / `agent channel start` thừa
