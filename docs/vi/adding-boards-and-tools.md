# Thêm Board và Tool — Hướng dẫn phần cứng Agent

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào Agent.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.agent/config.toml)
agent peripheral add nucleo-f401re /dev/ttyACM0
agent peripheral add arduino-uno /dev/cu.usbmodem12345
agent peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, Agent có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `agent peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `agent peripheral list` | Liệt kê các board đã cấu hình |
| `agent peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `agent peripheral flash` | Nạp firmware Arduino |
| `agent peripheral flash-nucleo` | Nạp firmware Nucleo |
| `agent hardware discover` | Liệt kê thiết bị USB |
| `agent hardware info` | Thông tin chip qua probe-rs |
