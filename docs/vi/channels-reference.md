...
Tài liệu này là nguồn tham khảo chính thức về cấu hình channel trong Agent.
...
6. **Trạng thái runtime cũ**: config đã thay đổi nhưng `agent daemon` chưa được khởi động lại.

---

## 1. Namespace cấu hình

Tất cả cài đặt channel nằm trong `channels_config` trong `~/.agent/config.toml`.
...
Khi chạy `agent channel start` (hoặc chế độ daemon), Telegram và Discord hỗ trợ chuyển đổi runtime theo phạm vi người gửi:
...
- Xem trước bộ nhớ cache model từ `agent models refresh --provider <ID>`.
- Đây là lệnh chat runtime, không phải lệnh con CLI.

## Giao thức marker hình ảnh đầu vào

Agent hỗ trợ đầu vào multimodal qua các marker nội tuyến trong tin nhắn:
...
Nếu `[channels_config.matrix]` có mặt nhưng binary được build mà không có `channel-matrix`, các lệnh `agent channel list`, `agent channel doctor`, và `agent channel start` sẽ ghi log rằng Matrix bị bỏ qua có chủ ý trong bản build này.
...
access_token = "syt_..."
user_id = "@agent:matrix.example.com"   # tùy chọn, khuyến nghị cho E2EE
...
Agent hỗ trợ hai backend WhatsApp:
...
5. Khởi động lại `agent daemon` sau khi thay đổi config.
...
```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Webhook|Channel" /tmp/agent.log
```
