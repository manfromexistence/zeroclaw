# Tài liệu tham khảo Providers — Agent
...
3. Biến môi trường dự phòng chung: `AGENT_API_KEY`, sau đó là `API_KEY`
...
- Sau khi chuẩn hóa multimodal, Agent gửi payload hình ảnh qua trường `messages[].images` gốc của Ollama.
- Nếu chọn provider không hỗ trợ vision, Agent trả về lỗi rõ ràng thay vì âm thầm bỏ qua hình ảnh.
...
- Khám phá model: `agent models refresh --provider nvidia`
