# Agent trên Arduino Uno Q — Hướng dẫn từng bước

Chạy Agent trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

Agent bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `agent peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
## Phase 2: Cài đặt Agent trên Uno Q
...
# Clone agent (hoặc scp project của bạn)
...
# Copy sang Uno Q
scp target/aarch64-unknown-linux-gnu/release/agent arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/agent /usr/local/bin/"
```
...
# Cấu hình nhanh
agent onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter
...
## Phase 4: Chạy Agent Daemon

```bash
ssh arduino@<UNO_Q_IP>

# Chạy daemon (Telegram polling hoạt động qua WiFi)
agent daemon --host 127.0.0.1 --port 3000
```

**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — Agent phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (Agent xử lý tự động)

Agent bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo agent):
```bash
agent peripheral setup-uno-q --host 192.168.0.48
```

**Từ Uno Q** (đã SSH vào):
```bash
agent peripheral setup-uno-q
```
...
Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, Agent dùng `gpio_write` qua Bridge.
...
| 5 | `git clone https://github.com/agent-labs/agent.git && cd agent` |
| 6 | `cargo build --release --no-default-features` |
| 7 | `agent onboard --api-key KEY --provider openrouter` |
| 8 | Chỉnh sửa `~/.agent/config.toml` (thêm Telegram bot_token) |
| 9 | `agent daemon --host 127.0.0.1 --port 3000` |
...
- **"command not found: agent"** — Dùng đường dẫn đầy đủ: `/usr/local/bin/agent` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`agent peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. Agent dùng endpoint v4 chính xác.
