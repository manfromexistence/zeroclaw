...
[INFO] ✓ Audit logging enabled (~/.config/agent/audit.log)
...
Security Status:
  ...
  ✓ Audit logging: ~/.config/agent/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ agent config set security.sandbox.enabled false

# Bật backend cụ thể
$ agent config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ agent config set security.resources.max_memory_mb 2048
```
...
✅ **Hiển thị trạng thái** — `agent security --status`
