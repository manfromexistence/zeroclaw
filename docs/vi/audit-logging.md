...
## Vấn đề
Agent ghi log các hành động nhưng thiếu audit trail chống giả mạo cho:
...
# Chống giả mạo
sign_events = true
signing_key_path = "~/.config/agent/audit.key"
...
agent audit --user @alice

# Hiển thị tất cả lệnh rủi ro cao
agent audit --risk high

# Hiển thị vi phạm trong 24 giờ qua
agent audit --since 24h --violations-only

# Xuất sang JSON để phân tích
agent audit --format json --output audit.json
