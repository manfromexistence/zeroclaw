...
| Tình huống | Lệnh |
|----------|---------|
| Có API key, muốn cài nhanh nhất | `agent onboard --api-key sk-... --provider openrouter` |
| Muốn được hướng dẫn từng bước | `agent onboard` |
| Đã có config, chỉ cần sửa kênh | `agent onboard --channels-only` |
...
- Thiết lập nhanh: `agent onboard --api-key "sk-..." --provider openrouter`
- Thiết lập hướng dẫn: `agent onboard`
- Kiểm tra môi trường: `agent status` + `agent doctor`
