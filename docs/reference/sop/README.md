...
- CLI `agent sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.agent/workspace/sops/deploy-prod/SOP.toml
   ~/.agent/workspace/sops/deploy-prod/SOP.md
   ```
