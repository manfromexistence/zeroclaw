...
3. Generic fallback env vars: `AGENT_API_KEY` then `API_KEY`
...
- After multimodal normalization, Agent sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, Agent returns a structured capability error instead of silently ignoring images.
...
- Agent normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `agent models refresh --provider llamacpp`
...
- Model discovery: `agent models refresh --provider sglang`
...
- Model discovery: `agent models refresh --provider vllm`
...
- Model discovery: `agent models refresh --provider osaurus`
...
- Model discovery: `agent models refresh --provider nvidia`
