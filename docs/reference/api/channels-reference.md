...
This document is the canonical reference for channel configuration in Agent.
...
6. **Stale runtime state**: config changed but `agent daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.agent/config.toml`.
...
When running `agent channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `agent models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

Agent supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `agent channel list`, `agent channel doctor`, and `agent channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@agent:matrix.example.com"   # optional, recommended for E2EE
...
Agent supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.agent/state/whatsapp-web/session.db"
...
- `AGENT_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `AGENT_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `agent daemon` after config changes.
...
```bash
RUST_LOG=info agent daemon 2>&1 | tee /tmp/agent.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/agent.log
```
