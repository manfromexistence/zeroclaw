...
Thanks for your interest in contributing to Agent! This guide will help you get started.
...
**Background:** Agent previously used `main` in some documentation and scripts, which caused 404 errors, broken CI refs, and contributor confusion (see [#2929](https://github.com/agent-labs/agent/issues/2929), [#3061](https://github.com/agent-labs/agent/issues/3061), [#3194](https://github.com/agent-labs/agent/pull/3194)). As of March 2026, all references have been corrected, stale branches cleaned up, and the `main` branch permanently deleted.
...
1. **Find an issue.** Look for issues labeled [`good first issue`](https://github.com/agent-labs/agent/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — these are scoped for newcomers and include context to get moving quickly.
...
4. **Start with Track A.** Agent uses three [collaboration tracks](#collaboration-tracks-risk-based) (A/B/C) based on risk. First-time contributors should target **Track A** (docs, tests, chore) — these require lighter review and are the fastest path to a merged PR.
...
Agent supports layered secret management for local development and CI hygiene.
...
2. **Config file** (`~/.agent/config.toml`)
    - Persistent setup for long-term use
    - When `secrets.encrypt = true` (default), secret values are encrypted before save
    - Secret key is stored at `~/.agent/.secret_key` with restricted permissions
    - Use `agent onboard` for guided setup
...
3. Generic env vars (`AGENT_API_KEY`, `API_KEY`)
...
- `~/.agent/.secret_key` or similar key files
...
- If identity-like wording is required, use Agent-centric labels only (for example: `AgentAgent`, `AgentOperator`, `agent_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use Agent/project-native roles instead of personal or real-world identities.
...
Agent's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use Agent-native identifiers only (`AgentAgent`, `agent_user`, `agent_node`).
...
- **Good identity-like label**: `AgentAgent`, `agent_user`, `agent_node`
...
- [ ] Any required identity-like wording uses Agent/project-native labels only
