...
<h1 align="center">Agent 🦀（Русский）</h1>
...
<p align="center">
  <a href="https://raw.githubusercontent.com/agent-labs/agent/master/install.sh">Установка в 1 клик</a> |
...
|---|---|---|---|
| 2026-02-19 | _Срочно_ | Мы **не аффилированы** с `openagen/agent` и `agent.org`. Домен `agent.org` сейчас указывает на fork `openagen/agent`, и этот домен/репозиторий выдают себя за наш официальный сайт и проект. | Не доверяйте информации, бинарникам, сборам средств и «официальным» объявлениям из этих источников. Используйте только [этот репозиторий](https://github.com/agent-labs/agent) и наши верифицированные соцсети. |
| 2026-02-21 | _Важно_ | Наш официальный сайт уже запущен: [agentlabs.ai](https://agentlabs.ai). Спасибо, что дождались запуска. При этом попытки выдавать себя за Agent продолжаются, поэтому не участвуйте в инвестициях, сборах средств и похожих активностях, если они не подтверждены через наши официальные каналы. | Ориентируйтесь только на [этот репозиторий](https://github.com/agent-labs/agent); также следите за [X (@agentlabs)](https://x.com/agentlabs?s=21), [Telegram (@agentlabs)](https://t.me/agentlabs), [Facebook (группа)](https://www.facebook.com/groups/agentlabs), [Reddit (r/agentlabs)](https://www.reddit.com/r/agentlabs/) и [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) для официальных обновлений. |
...
Agent — это производительная и расширяемая инфраструктура автономного AI-агента. Agent — это **операционная система времени выполнения** для агентных рабочих процессов — инфраструктура, абстрагирующая модели, инструменты, память и выполнение, позволяя создавать агентов один раз и запускать где угодно.
...
## Снимок бенчмарка (Agent vs OpenClaw, воспроизводимо)
...
> Примечание: результаты Agent получены на release-сборке с помощью `/usr/bin/time -l`. OpenClaw требует Node.js runtime; только этот runtime обычно добавляет около 390MB дополнительного потребления памяти. NanoBot требует Python runtime. PicoClaw и Agent — статические бинарники.

<p align="center">
  <img src="docs/assets/agent-comparison.jpeg" alt="Сравнение Agent и OpenClaw" width="800" />
</p>
...
- `agent --help`: ~`0.02s`, пик памяти ~`3.9MB`
- `agent status`: ~`0.01s`, пик памяти ~`4.1MB`
...
agent onboard --api-key sk-... --provider openrouter
agent onboard
...
Agent поддерживает нативные профили авторизации на основе подписки (мультиаккаунт, шифрование при хранении).

- Файл хранения: `~/.agent/auth-profiles.json`
- Ключ шифрования: `~/.agent/.secret_key`
