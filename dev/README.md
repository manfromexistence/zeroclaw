...
A fully containerized development sandbox for Agent agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`agent-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `agent` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/agent-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.agent` (mounted to `/agent-data/.agent`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.agent` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `agent-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=agent-target`) to speed repeat local image builds.
