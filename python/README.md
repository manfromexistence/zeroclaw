...
Python companion package for [Agent](https://github.com/agent-labs/agent) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install agent-tools[discord]
```
...
# Run the CLI
agent-tools "List files in the current directory"
...
| Feature | Rust Agent | agent-tools |
|---------|---------------|----------------|
...
Use **Rust Agent** for production edge deployments. Use **agent-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
