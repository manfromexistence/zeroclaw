...
<p align="center">
  Agent je <strong>operační systém runtime</strong> pro workflow agentů — infrastruktura která abstrahuje modely, nástroje, paměť a provádění pro stavbu agentů jednou a spouštění kdekoliv.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritické_  | **Nejsme propojeni** s `openagen/agent` nebo `agent.org`. Doména `agent.org` aktuálně směřuje na fork `openagen/agent`, a tato doména/repoziťář se vydává za náš oficiální web/projekt.                                                                                                                                                                                 | Nevěřte informacím, binárním souborům, fundraisingu nebo oznámením z těchto zdrojů. Používejte pouze [tento repoziťář](https://github.com/agent-labs/agent) a naše ověřené sociální účty.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Důležité_ | Náš oficiální web je nyní online: [agentlabs.ai](https://agentlabs.ai). Děkujeme za trpělivost během čekání. Stále detekujeme pokusy o vydávání se: neúčastněte žádné investiční/fundraisingové aktivity ve jménu Agent pokud není publikována přes naše oficiální kanály.                                                                                                                   | Používejte [tento repoziťář](https://github.com/agent-labs/agent) jako jediný zdroj pravdy. Sledujte [X (@agentlabs)](https://x.com/agentlabs?s=21), [Telegram (@agentlabs)](https://t.me/agentlabs), [Facebook (skupina)](https://www.facebook.com/groups/agentlabs), [Reddit (r/agentlabs)](https://www.reddit.com/r/agentlabs/), a [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) pro oficiální aktualizace. |
...
## Benchmark Snapshot (Agent vs OpenClaw, Reprodukovatelné)
...
> Poznámky: Výsledky Agent jsou měřeny na produkčních buildech pomocí `/usr/bin/time -l`. OpenClaw vyžaduje Node.js runtime (typicky ~390 MB dodatečného paměťového režijního nákladu), zatímco NanoBot vyžaduje Python runtime. PicoClaw a Agent jsou statická binaria. Výše uvedené RAM čísla jsou runtime paměť; build-time kompilační požadavky jsou vyšší.

<p align="center">
  <img src="zero-claw.jpeg" alt="Porovnání Agent vs OpenClaw" width="800" />
</p>
...
- `agent --help`: reálný čas přibližně `0.02s`, špičková paměťová stopa ~`3.9 MB`
- `agent status`: reálný čas přibližně `0.01s`, špičková paměťová stopa ~`4.1 MB`
...
Skript `bootstrap.sh` nainstaluje Rust, naklonuje Agent, zkompiluje ho a nastaví vaše počáteční vývojové prostředí:
...
2. Naklonuje Agent repoziťář
3. Zkompiluje Agent v release módu
4. Nainstaluje `agent` do `~/.cargo/bin/`
5. Vytvoří výchozí workspace strukturu v `~/.agent/workspace/`
6. Vygeneruje počáteční konfigurační soubor `~/.agent/workspace/config.toml`

Po bootstrapu znovu načtěte váš shell nebo spusťte `source ~/.cargo/env` pro použití příkazu `agent` globálně.
...
1. Nakonfigurujte své AI poskytovatele v `~/.agent/workspace/config.toml`
2. Podívejte se na [konfigurační referenci](docs/config-reference.md) pro pokročilé možnosti
3. Spusťte agenta: `agent agent start`
4. Otestujte přes váš preferovaný kanál (viz [kanálová reference](docs/channels-reference.md))

## Konfigurace

Upravte `~/.agent/workspace/config.toml` pro konfiguraci poskytovatelů, kanálů a chování systému.
...
Agent podporuje dva backendy provádění kódu:
...
```bash
# Správa workspace
agent init                # Inicializuje nový workspace
agent status              # Zobrazuje stav daemon/agent
agent config validate     # Ověřuje syntaxi a hodnoty config.toml

# Správa daemon
agent daemon start        # Spouští daemon na pozadí
agent daemon stop         # Zastavuje běžící daemon
agent daemon restart      # Restartuje daemon (znovunačtení config)
agent daemon logs         # Zobrazuje daemon logy

# Správa agent
agent agent start         # Spouští agenta (vyžaduje běžící daemon)
agent agent stop          # Zastavuje agenta
agent agent restart       # Restartuje agenta (znovunačtení config)

# Párovací operace
agent pairing init        # Generuje nové párovací tajemství
agent pairing rotate      # Rotuje existující párovací tajemství

# Tunneling (pro veřejnou expozici)
agent tunnel start        # Spouští tunnel k lokálnímu daemon
agent tunnel stop         # Zastavuje aktivní tunnel

# Diagnostika
agent doctor              # Spouští kontroly zdraví systému
agent version             # Zobrazuje verzi a build informace
```
...
Pokud Agent pomáhá vaší práci a chcete podpořit pokračující vývoj, můžete darovat zde:
...
**Toto je jediný oficiální Agent repoziťář:**

> <https://github.com/agent-labs/agent>

Jakýkoliv jiný repoziťář, organizace, doména nebo balík tvrdící že je "Agent" nebo naznačující afiliaci s Agent Labs je **neautorizovaný a není spojen s tímto projektem**. Známé neautorizované forky budou uvedeny v [TRADEMARK.md](TRADEMARK.md).

Pokud narazíte na vydávání se nebo zneužití ochranné známky, prosím [otevřete issue](https://github.com/agent-labs/agent/issues).

---

## Licence

Agent je duálně licencován pro maximální otevřenost a ochranu přispěvatelů:
...
Název **Agent** a logo jsou registrované ochranné známky Agent Labs. Tato licence neuděluje povolení je používat k naznačení schválení nebo afiliace. Viz [TRADEMARK.md](TRADEMARK.md) pro povolená a zakázaná použití.
...
- Nová `Skill` → `~/.agent/workspace/skills/<n>/`

---

**Agent** — Nulová režie. Nulové kompromisy. Nasazujte kdekoliv. Měňte cokoliv. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#agent-labs/agent&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Graf Historie Hvězd" src="https://api.star-history.com/svg?repos=agent-labs/agent&type=date&legend=top-left" />
    </picture>
  </a>
</p>
