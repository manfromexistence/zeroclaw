...
<p align="center">
  ...
  <a href="https://raw.githubusercontent.com/agent-labs/agent/master/install.sh">Cài đặt một lần bấm</a> |
...
<p align="center">
  Agent là <strong>hệ điều hành runtime</strong> cho các quy trình làm việc của tác tử — cơ sở hạ tầng trừu tượng hóa mô hình, công cụ, bộ nhớ và thực thi để xây dựng tác tử một lần và chạy ở mọi nơi.
</p>
...
|---|---|---|---|
| 2026-02-19 | _Nghiêm trọng_ | Chúng tôi **không có liên kết** với `openagen/agent` hoặc `agent.org`. Tên miền `agent.org` hiện đang trỏ đến fork `openagen/agent`, và tên miền/repository đó đang mạo danh website/dự án chính thức của chúng tôi. | Không tin tưởng thông tin, binary, gây quỹ, hay thông báo từ các nguồn đó. Chỉ sử dụng [repository này](https://github.com/agent-labs/agent) và các tài khoản mạng xã hội đã được xác minh của chúng tôi. |
| 2026-02-21 | _Quan trọng_ | Website chính thức của chúng tôi đã ra mắt: [agentlabs.ai](https://agentlabs.ai). Cảm ơn mọi người đã kiên nhẫn chờ đợi. Chúng tôi vẫn đang ghi nhận các nỗ lực mạo danh, vì vậy **không** tham gia bất kỳ hoạt động đầu tư hoặc gây quỹ nào nhân danh Agent nếu thông tin đó không được công bố qua các kênh chính thức của chúng tôi. | Sử dụng [repository này](https://github.com/agent-labs/agent) làm nguồn thông tin duy nhất đáng tin cậy. Theo dõi [X (@agentlabs)](https://x.com/agentlabs?s=21), [Facebook (nhóm)](https://www.facebook.com/groups/agentlabs), và [Reddit (r/agentlabs)](https://www.reddit.com/r/agentlabs/) để nhận cập nhật chính thức. |
...
## So sánh hiệu suất (Agent vs OpenClaw, có thể tái tạo)
...
> Ghi chú: Kết quả Agent được đo trên release build sử dụng `/usr/bin/time -l`. OpenClaw yêu cầu runtime Node.js (thường thêm ~390MB bộ nhớ overhead), còn NanoBot yêu cầu runtime Python. PicoClaw và Agent là các static binary. Số RAM ở trên là bộ nhớ runtime; yêu cầu biên dịch lúc build-time sẽ cao hơn.

<p align="center">
  <img src="docs/assets/agent-comparison.jpeg" alt="Agent vs OpenClaw Comparison" width="800" />
</p>
...
- `agent --help`: khoảng `0.02s`, bộ nhớ đỉnh ~`3.9MB`
- `agent status`: khoảng `0.01s`, bộ nhớ đỉnh ~`4.1MB`
...
Hoặc bỏ qua các bước trên, cài hết mọi thứ (system deps, Rust, Agent) chỉ bằng một lệnh:

```bash
curl -LsSf https://raw.githubusercontent.com/agent-labs/agent/master/install.sh | bash
```
...
<https://github.com/agent-labs/agent/releases/latest>
...
install -m 0755 agent "$HOME/.cargo/bin/agent"
```
...
agent gateway                # mặc định: 127.0.0.1:42617
agent gateway --port 0       # cổng ngẫu nhiên (tăng cường bảo mật)
...
Agent hỗ trợ profile xác thực theo gói đăng ký (đa tài khoản, mã hóa khi lưu).

- File lưu trữ: `~/.agent/auth-profiles.json`
- Khóa mã hóa: `~/.agent/.secret_key`
...
agent auth use --provider openai-codex --profile work
```
...
# Lệnh alias
agent auth setup-token --provider anthropic --profile default
```
...
agent agent --provider anthropic -m "hello"
```
...
Nếu Agent giúp ích cho công việc của bạn và bạn muốn hỗ trợ phát triển liên tục, bạn có thể quyên góp tại đây:
...
**Đây là repository Agent chính thức duy nhất:**
> <https://github.com/agent-labs/agent>

Bất kỳ repository, tổ chức, tên miền hay gói nào khác tuyên bố là "Agent" hoặc ngụ ý liên kết với Agent Labs đều là **không được ủy quyền và không liên kết với dự án này**. Các fork không được ủy quyền đã biết sẽ được liệt kê trong [TRADEMARK.md](docs/maintainers/trademark.md).

Nếu bạn phát hiện hành vi mạo danh hoặc lạm dụng nhãn hiệu, vui lòng [mở một issue](https://github.com/agent-labs/agent/issues).

---

## Giấy phép

Agent được cấp phép kép để tối đa hóa tính mở và bảo vệ người đóng góp:
...
Tên **Agent** và logo là nhãn hiệu của Agent Labs. Giấy phép này không cấp phép sử dụng chúng để ngụ ý chứng thực hoặc liên kết. Xem [TRADEMARK.md](docs/maintainers/trademark.md) để biết các sử dụng được phép và bị cấm.
...
- `Skill` mới → `~/.agent/workspace/skills/<name>/`

---

**Agent** — Không tốn thêm tài nguyên. Không đánh đổi. Triển khai ở đâu cũng được. Thay thế gì cũng được. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#agent-labs/agent&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=agent-labs/agent&type=date&legend=top-left" />
    </picture>
  </a>
</p>
