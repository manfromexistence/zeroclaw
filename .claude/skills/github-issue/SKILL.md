...
File a structured GitHub issue (bug report or feature request) for Agent interactively from Claude Code.

## When to Use

Trigger when the user wants to file a GitHub issue, report a bug, or request a feature for Agent. Keywords: "file issue", "report bug", "feature request", "open issue", "create issue", "github issue".

## Instructions

You are filing a GitHub issue against the Agent repository using structured issue forms. Follow this workflow exactly.
...
cargo metadata --format-version=1 --no-deps 2>/dev/null | jq -r '.packages[] | select(.name=="agent") | .version' 2>/dev/null   # Agent version
...
- **Use neutral project-scoped placeholders** per Agent's privacy contract.
