...
<p align="center">
  Agent es el <strong>sistema operativo de runtime</strong> para flujos de trabajo de agentes — una infraestructura que abstrae modelos, herramientas, memoria y ejecución para construir agentes una vez y ejecutarlos en cualquier lugar.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Crítico_  | **No estamos afiliados** con `openagen/agent` o `agent.org`. El dominio `agent.org` apunta actualmente al fork `openagen/agent`, y este dominio/repositorio está suplantando nuestro sitio web/proyecto oficial.                                                                                                                                                                                 | No confíes en información, binarios, recaudaciones de fondos o anuncios de estas fuentes. Usa solo [este repositorio](https://github.com/agent-labs/agent) y nuestras cuentas sociales verificadas.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Importante_ | Nuestro sitio web oficial ahora está en línea: [agentlabs.ai](https://agentlabs.ai). Gracias por tu paciencia durante la espera. Todavía detectamos intentos de suplantación: no participes en ninguna actividad de inversión/financiamiento en nombre de Agent si no se publica a través de nuestros canales oficiales.                                                                                                                   | Usa [este repositorio](https://github.com/agent-labs/agent) como la única fuente de verdad. Sigue [X (@agentlabs)](https://x.com/agentlabs?s=21), [Telegram (@agentlabs)](https://t.me/agentlabs), [Facebook (grupo)](https://www.facebook.com/groups/agentlabs), [Reddit (r/agentlabs)](https://www.reddit.com/r/agentlabs/), y [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) para actualizaciones oficiales. |
...
### Por qué los equipos eligen Agent
...
## Instantánea de Benchmark (Agent vs OpenClaw, Reproducible)
...
> Notas: Los resultados de Agent se miden en builds de producción usando `/usr/bin/time -l`. OpenClaw requiere el runtime Node.js (típicamente ~390 MB de sobrecarga de memoria adicional), mientras que NanoBot requiere el runtime Python. PicoClaw y Agent son binarios estáticos. Las cifras de RAM anteriores son memoria de runtime; los requisitos de compilación en tiempo de build son mayores.

<p align="center">
  <img src="zero-claw.jpeg" alt="Comparación Agent vs OpenClaw" width="800" />
</p>
...
- `agent --help`: tiempo real aprox `0.02s`, huella de memoria máxima ~`3.9 MB`
- `agent status`: tiempo real aprox `0.01s`, huella de memoria máxima ~`4.1 MB`
...
El script `bootstrap.sh` instala Rust, clona Agent, lo compila, y configura tu entorno de desarrollo inicial:
...
2. Clonará el repositorio Agent
3. Compilará Agent en modo release
4. Instalará `agent` en `~/.cargo/bin/`
5. Creará la estructura de workspace por defecto en `~/.agent/workspace/`
6. Generará un archivo de configuración inicial `~/.agent/workspace/config.toml`

Después del bootstrap, recarga tu shell o ejecuta `source ~/.cargo/env` para usar el comando `agent` globalmente.
...
1. Configura tus proveedores de AI en `~/.agent/workspace/config.toml`
2. Revisa la [referencia de configuración](docs/config-reference.md) para opciones avanzadas
3. Inicia el agente: `agent agent start`
4. Prueba vía tu canal preferido (ver [referencia de canales](docs/channels-reference.md))

## Configuración

Edita `~/.agent/workspace/config.toml` para configurar proveedores, canales y comportamiento del sistema.
...
Agent soporta dos backends de ejecución de código:
...
agent init                # Inicializa un nuevo workspace
agent status              # Muestra estado de daemon/agent
agent config validate     # Verifica sintaxis y valores de config.toml

# Gestión de daemon
agent daemon start        # Inicia el daemon en segundo plano
agent daemon stop         # Detiene el daemon en ejecución
agent daemon restart      # Reinicia el daemon (recarga de config)
agent daemon logs         # Muestra logs del daemon

# Gestión de agent
agent agent start         # Inicia el agent (requiere daemon ejecutándose)
agent agent stop          # Detiene el agent
agent agent restart       # Reinicia el agent (recarga de config)

# Operaciones de emparejamiento
agent pairing init        # Genera un nuevo secreto de emparejamiento
agent pairing rotate      # Rota el secreto de emparejamiento existente

# Tunneling (para exposición pública)
agent tunnel start        # Inicia un tunnel hacia el daemon local
agent tunnel stop         # Detiene el tunnel activo

# Diagnóstico
agent doctor              # Ejecuta verificaciones de salud del sistema
agent version             # Muestra versión e información de build
```
...
Si Agent ayuda a tu trabajo y deseas apoyar el desarrollo continuo, puedes donar aquí:
...
**Este es el único repositorio oficial de Agent:**

> <https://github.com/agent-labs/agent>

Cualquier otro repositorio, organización, dominio o paquete que afirme ser "Agent" o que implique afiliación con Agent Labs es **no autorizado y no está afiliado con este proyecto**. Los forks no autorizados conocidos serán listados en [TRADEMARK.md](TRADEMARK.md).

Si encuentras suplantación o uso indebido de marca, por favor [abre un issue](https://github.com/agent-labs/agent/issues).

---

## Licencia

Agent tiene doble licencia para máxima apertura y protección de contribuyentes:
...
El nombre **Agent** y el logo son marcas registradas de Agent Labs. Esta licencia no otorga permiso para usarlos para implicar aprobación o afiliación. Ver [TRADEMARK.md](TRADEMARK.md) para usos permitidos y prohibidos.
...
- Nueva `Skill` → `~/.agent/workspace/skills/<n>/`

---

**Agent** — Cero sobrecarga. Cero compromiso. Despliega en cualquier lugar. Intercambia cualquier cosa. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#agent-labs/agent&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Gráfico de Historial de Estrellas" src="https://api.star-history.com/svg?repos=agent-labs/agent&type=date&legend=top-left" />
    </picture>
  </a>
</p>
