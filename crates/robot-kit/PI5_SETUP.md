...
Complete guide to setting up a Agent-powered robot on Raspberry Pi 5.
...
cp models/ggml-base.bin ~/.agent/models/
```
...
mkdir -p ~/.agent/models/piper
cd ~/.agent/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.agent/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build Agent Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/agent-labs/agent
cd agent

# Build robot kit
cargo build --release -p agent-robot-kit

# Build main agent (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.agent
nano ~/.agent/robot.toml
```

```toml
# ~/.agent/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.agent/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/agent_sensors.fifo"
...
```bash
# Configure Agent to use robot tools
...
cp ~/agent/crates/robot-kit/SOUL.md ~/.agent/workspace/
...
# Start sensor loop
if [ ! -p /tmp/agent_sensors.fifo ]; then
    mkfifo /tmp/agent_sensors.fifo
fi
...
```bash
# /etc/systemd/system/agent-robot.service
sudo tee /etc/systemd/system/agent-robot.service << 'EOF'
[Unit]
...
sudo systemctl enable agent-robot
sudo systemctl start agent-robot

# Check status
sudo systemctl status agent-robot
journalctl -u agent-robot -f  # View logs
```
