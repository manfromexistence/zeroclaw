...
<p align="center">
  Ang Agent ay ang <strong>runtime operating system</strong> para sa agent workflows — isang infrastructure na nag-a-abstract ng mga modelo, tools, memory, at execution upang bumuo ng mga agent nang isang beses at patakbuhin ang mga ito kahit saan.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritikal_  | **Hindi kami kaugnay** sa `openagen/agent` o `agent.org`. Ang domain na `agent.org` ay kasalukuyang tumuturo sa fork na `openagen/agent`, at ang domain/repository na ito ay nanggagaya sa aming opisyal na website/proyekto.                                                                                                                                                                                 | Huwag magtiwala sa impormasyon, binaries, fundraising, o mga anunsyo mula sa mga pinagmulang ito. Gamitin lamang [ang repository na ito](https://github.com/agent-labs/agent) at aming mga verified social media accounts.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Mahalaga_ | Ang aming opisyal na website ay ngayon online: [agentlabs.ai](https://agentlabs.ai). Salamat sa iyong pasensya sa panahon ng paghihintay. Nakikita pa rin namin ang mga pagtatangka ng panliliko: huwag lumahok sa anumang investment/funding activity sa ngalan ng Agent kung hindi ito nai-publish sa pamamagitan ng aming mga opisyal na channel.                                                                                                                   | Gamitin [ang repository na ito](https://github.com/agent-labs/agent) bilang nag-iisang source of truth. Sundan [X (@agentlabs)](https://x.com/agentlabs?s=21), [Telegram (@agentlabs)](https://t.me/agentlabs), [Facebook (grupo)](https://www.facebook.com/groups/agentlabs), [Reddit (r/agentlabs)](https://www.reddit.com/r/agentlabs/), at [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) para sa mga opisyal na update. |
...
### Bakit pinipili ng mga team ang Agent
...
## Benchmark Snapshot (Agent vs OpenClaw, Reproducible)
...
> Mga Tala: Ang mga resulta ng Agent ay sinusukat sa production builds gamit ang `/usr/bin/time -l`. Ang OpenClaw ay nangangailangan ng Node.js runtime (typically ~390 MB additional memory overhead), habang ang NanoBot ay nangangailangan ng Python runtime. Ang PicoClaw at Agent ay static binaries. Ang mga RAM figure sa itaas ay runtime memory; ang build-time compilation requirements ay mas mataas.

<p align="center">
  <img src="zero-claw.jpeg" alt="Agent vs OpenClaw Comparison" width="800" />
</p>
...
- `agent --help`: real time na humigit-kumulang `0.02s`, peak memory footprint ~`3.9 MB`
- `agent status`: real time na humigit-kumulang `0.01s`, peak memory footprint ~`4.1 MB`
...
Ang `bootstrap.sh` script ay nag-i-install ng Rust, nagi-clone ng Agent, nagi-compile, at nagse-set up ng iyong paunang development environment:
...
2. Magi-clone ng Agent repository
3. Magi-compile ng Agent sa release mode
4. Mag-i-install ng `agent` sa `~/.cargo/bin/`
5. Gagawa ng default workspace structure sa `~/.agent/workspace/`
6. Gagawa ng paunang configuration file na `~/.agent/workspace/config.toml`

Pagkatapos ng bootstrap, i-reload ang iyong shell o patakbuhin ang `source ~/.cargo/env` para gamitin ang `agent` command globally.
...
1. I-configure ang iyong AI providers sa `~/.agent/workspace/config.toml`
2. Tingnan ang [configuration reference](docs/config-reference.md) para sa advanced options
3. Simulan ang agent: `agent agent start`
4. I-test sa pamamagitan ng iyong preferred channel (tingnan ang [channels reference](docs/channels-reference.md))

## Configuration

I-edit ang `~/.agent/workspace/config.toml` para i-configure ang providers, channels, at system behavior.
...
Sinusuportahan ng Agent ang dalawang code execution backends:
...
agent init                # Nag-initialize ng bagong workspace
agent status              # Nagpapakita ng daemon/agent status
agent config validate     # Nag-verify ng config.toml syntax at values

# Daemon management
agent daemon start        # Nagse-start ng daemon sa background
agent daemon stop         # Naghihinto sa running daemon
agent daemon restart      # Nagre-restart ng daemon (config reload)
agent daemon logs         # Nagpapakita ng daemon logs

# Agent management
agent agent start         # Nagse-start ng agent (nangangailangan ng running daemon)
agent agent stop          # Naghihinto sa agent
agent agent restart       # Nagre-restart ng agent (config reload)

# Pairing operations
agent pairing init        # Nag-generate ng bagong pairing secret
agent pairing rotate      # Nag-rotate ng existing pairing secret

# Tunneling (para sa public exposure)
agent tunnel start        # Nagse-start ng tunnel sa local daemon
agent tunnel stop         # Naghihinto sa active tunnel

# Diagnostics
agent doctor              # Nagpapatakbo ng system health checks
agent version             # Nagpapakita ng version at build info
```
...
## Suportahan ang Agent

Kung tinutulungan ng Agent ang iyong trabaho at nais mong suportahan ang patuloy na development, maaari kang mag-donate dito:
...
**Ito ang tanging opisyal na Agent repository:**

> <https://github.com/agent-labs/agent>

Ang anumang iba pang repository, organization, domain, o package na nagpapanggap na "Agent" o nagpapahiwatig ng affiliation sa Agent Labs ay **hindi awtorisado at hindi kaugnay sa proyektong ito**. Ang mga kilalang unauthorized forks ay ililista sa [TRADEMARK.md](TRADEMARK.md).

Kung makakita ka ng impersonation o trademark misuse, mangyaring [magbukas ng isyu](https://github.com/agent-labs/agent/issues).

---

## License

Ang Agent ay dual-licensed para sa maximum openness at contributor protection:
...
Ang pangalang **Agent** at logo ay mga rehistradong trademark ng Agent Labs. Ang license na ito ay hindi nagbibigay ng pahintulot na gamitin ang mga ito upang ipahiwatig ang endorsement o affiliation. Tingnan ang [TRADEMARK.md](TRADEMARK.md) para sa mga allowed at prohibited uses.
...
- Bagong `Skill` → `~/.agent/workspace/skills/<n>/`
...
<p align="center">
  <a href="https://www.star-history.com/#agent-labs/agent&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Star History Graph" src="https://api.star-history.com/svg?repos=agent-labs/agent&type=date&legend=top-left" />
    </picture>
  </a>
</p>
