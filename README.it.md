...
<p align="center">
  Agent è il <strong>sistema operativo runtime</strong> per i workflow degli agenti — un'infrastruttura che astrae modelli, strumenti, memoria ed esecuzione per costruire agenti una volta e eseguirli ovunque.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critico_  | **Non siamo affiliati** con `openagen/agent` o `agent.org`. Il dominio `agent.org` punta attualmente al fork `openagen/agent`, e questo dominio/repository sta contraffacendo il nostro sito web/progetto ufficiale.                                                                                                                                                                                 | Non fidarti di informazioni, binari, raccolte fondi o annunci da queste fonti. Usa solo [questo repository](https://github.com/agent-labs/agent) e i nostri account social verificati.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Importante_ | Il nostro sito ufficiale è ora online: [agentlabs.ai](https://agentlabs.ai). Grazie per la pazienza durante l'attesa. Rileviamo ancora tentativi di contraffazione: non partecipare ad alcuna attività di investimento/finanziamento a nome di Agent se non pubblicata tramite i nostri canali ufficiali.                                                                                                                   | Usa [questo repository](https://github.com/agent-labs/agent) come unica fonte di verità. Segui [X (@agentlabs)](https://x.com/agentlabs?s=21), [Telegram (@agentlabs)](https://t.me/agentlabs), [Facebook (gruppo)](https://www.facebook.com/groups/agentlabs), [Reddit (r/agentlabs)](https://www.reddit.com/r/agentlabs/), e [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) per aggiornamenti ufficiali. |
...
## Snapshot Benchmark (Agent vs OpenClaw, Riproducibile)
...
> Note: I risultati di Agent sono misurati su build di produzione usando `/usr/bin/time -l`. OpenClaw richiede il runtime Node.js (tipicamente ~390 MB di overhead memoria aggiuntivo), mentre NanoBot richiede il runtime Python. PicoClaw e Agent sono binari statici. Le cifre RAM sopra sono memoria runtime; i requisiti di compilazione in build-time sono maggiori.

<p align="center">
  <img src="zero-claw.jpeg" alt="Confronto Agent vs OpenClaw" width="800" />
</p>
...
- `agent --help`: tempo reale circa `0.02s`, impatto memoria massimo ~`3.9 MB`
- `agent status`: tempo reale circa `0.01s`, impatto memoria massimo ~`4.1 MB`
...
Lo script `bootstrap.sh` installa Rust, clona Agent, lo compila, e configura il tuo ambiente di sviluppo iniziale:
...
2. Clonerà il repository Agent
3. Compilerà Agent in modalità release
4. Installerà `agent` in `~/.cargo/bin/`
5. Creerà la struttura del workspace di default in `~/.agent/workspace/`
6. Genererà un file di configurazione iniziale `~/.agent/workspace/config.toml`

Dopo il bootstrap, ricarica la tua shell o esegui `source ~/.cargo/env` per usare il comando `agent` globalmente.
...
1. Configura i tuoi provider AI in `~/.agent/workspace/config.toml`
2. Controlla la [riferimento configurazione](docs/config-reference.md) per opzioni avanzate
3. Avvia l'agente: `agent agent start`
4. Testa tramite il tuo canale preferito (vedi [riferimento canali](docs/channels-reference.md))

## Configurazione

Modifica `~/.agent/workspace/config.toml` per configurare provider, canali e comportamento del sistema.
...
Agent supporta due backend di esecuzione del codice:
...
```bash
# Gestione workspace
agent init                # Inizializza un nuovo workspace
agent status              # Mostra stato daemon/agent
agent config validate     # Verifica sintassi e valori di config.toml

# Gestione daemon
agent daemon start        # Avvia il daemon in background
agent daemon stop         # Ferma il daemon in esecuzione
agent daemon restart      # Riavvia il daemon (ricaricamento config)
agent daemon logs         # Mostra log del daemon

# Gestione agent
agent agent start         # Avvia l'agent (richiede daemon in esecuzione)
agent agent stop          # Ferma l'agent
agent agent restart       # Riavvia l'agent (ricaricamento config)

# Operazioni di pairing
agent pairing init        # Genera un nuovo segreto di pairing
agent pairing rotate      # Ruota il segreto di pairing esistente

# Tunneling (per esposizione pubblica)
agent tunnel start        # Avvia un tunnel verso il daemon locale
agent tunnel stop         # Ferma il tunnel attivo

# Diagnostica
agent doctor              # Esegue controlli di salute del sistema
agent version             # Mostra versione e informazioni di build
```
...
Se Agent aiuta il tuo lavoro e desideri supportare lo sviluppo continuo, puoi donare qui:
...
**Questo è l'unico repository ufficiale di Agent:**

> <https://github.com/agent-labs/agent>

Qualsiasi altro repository, organizzazione, dominio o pacchetto che afferma di essere "Agent" o che implica affiliazione con Agent Labs è **non autorizzato e non affiliato a questo progetto**. I fork non autorizzati noti saranno elencati in [TRADEMARK.md](TRADEMARK.md).

Se incontri contraffazione o uso improprio del marchio, per favore [apri una issue](https://github.com/agent-labs/agent/issues).

---

## Licenza

Agent è doppia licenza per massima apertura e protezione dei contributori:
...
Il nome **Agent** e il logo sono marchi registrati di Agent Labs. Questa licenza non concede il permesso di usarli per implicare approvazione o affiliazione. Vedi [TRADEMARK.md](TRADEMARK.md) per usi permessi e proibiti.
...
- Nuova `Skill` → `~/.agent/workspace/skills/<n>/`

---

**Agent** — Zero overhead. Zero compromesso. Distribuisci ovunque. Scambia qualsiasi cosa. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#agent-labs/agent&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Grafico Storico Stelle" src="https://api.star-history.com/svg?repos=agent-labs/agent&type=date&legend=top-left" />
    </picture>
  </a>
</p>
