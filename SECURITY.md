...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/agent-labs/agent/security/advisories/new)
...
Agent implements defense-in-depth security:
...
Agent Docker images follow CIS Docker Benchmark best practices:
...
docker inspect --format='{{.Config.User}}' agent
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace agent gateway
```
